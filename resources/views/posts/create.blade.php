...
<div class="bg-card text-card-foreground border border-border rounded p-6 max-w-2xl mx-auto">
    ...
    <form action="{{ route('posts.store') }}" method="POST">
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-primary text-primary-foreground font-bold text-sm px-4 py-2 rounded border border-primary hover:opacity-90 transition-opacity duration-150">Publish Post</button>
            <a href="{{ route('posts.index') }}" class="bg-secondary text-secondary-foreground font-bold text-sm px-4 py-2 rounded border border-border hover:opacity-90 transition-opacity duration-150">Cancel</a>
        </div>
    </form>
</div>
@endsection
