...
@php
$resolvedId = $id ?? ($name ? str_replace(['[', ']'], ['-', ''], (string) $name) : null);
$previewId = $resolvedId ? $resolvedId.'-preview' : \Illuminate\Support\Str::uuid()->toString();
$previewSlot = null;
...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-2" data-markdown-editor data-markdown-preview-id="{{ $previewId }}">
    <div class="space-y-2">
    ...
    @if($previewSlot)
        {{ $previewSlot }}
    @else
        <livewire:markdown.preview :content="$value" :preview-id="$previewId" />
    @endif
</div>
