 ...
 * All image processing (crop/resize/convert) happens on the client-side.
 * This service stores processed base64 image data and writes files to disk.
 */
class ImageProcessor
{
    /**
     * Process and store a base64 image from the client.
     *
     * Accepted formats:
     ...
     * @return string The storage path of the image file
     */
    public function process(string $base64Data, string $directory = 'photos'): string
    {
        // Extract mime type from data URI.
        if (!preg_match('/^data:image\/(webp|png|jpeg|jpg);base64,/', $base64Data, $matches)) {
            throw new \InvalidArgumentException('Invalid base64 image data');
        }

        $mimeSubtype = strtolower($matches[1]);
        $extension = $mimeSubtype === 'jpg' ? 'jpg' : ($mimeSubtype === 'jpeg' ? 'jpg' : $mimeSubtype);
     ...
     * @return string The storage path of the image
     */
